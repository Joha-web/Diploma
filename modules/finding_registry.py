...
CWE_BY_TYPE: dict[str, str] = {
    ...
    "server_error_disclosure": "CWE-209",
...
FINDING_DEFINITIONS: dict[str, FindingDefinition] = {
    ...
    "xss_xsstrike_confirmed": FindingDefinition(
        "xss_xsstrike_confirmed",
        "Cross-Site Scripting confirmed by XSStrike",
        "HIGH",
        0.82,
        "xss",
        "XSStrike generated a payload that the page reflected at full efficiency.",
        REFERENCE_SETS["xss"],
        "active",
        ("xss", "reflected", "xsstrike"),
    ),
    "xss_xsstrike_reported": FindingDefinition(
        "xss_xsstrike_reported",
        "XSStrike reported a candidate XSS reflection",
        "MEDIUM",
        0.60,
        "xss",
        "XSStrike reflected a payload but did not confirm an unfiltered, executable context.",
        REFERENCE_SETS["xss"],
        "active",
        ("xss", "reflected", "xsstrike"),
    ),
    "xss_xsser_reported": FindingDefinition(
        "xss_xsser_reported",
        "XSSer reported an XSS injection",
        "MEDIUM",
        0.58,
        "xss",
        "XSSer reported a successful injection. XSSer is prone to false positives — confirm manually.",
        REFERENCE_SETS["xss"],
        "active",
        ("xss", "reflected", "xsser"),
    ),
    ...
    "command_injection_detected": FindingDefinition(
        "command_injection_detected",
        "OS command injection confirmed",
        "CRITICAL",
        0.90,
        "command_injection",
        "An injected payload executed as an OS command on the server (confirmed by an in-band probe, commix, or ffuf).",
        ("https://owasp.org/www-community/attacks/Command_Injection",
         "https://portswigger.net/web-security/os-command-injection"),
        "confirmed",
        ("command-injection", "rce", "confirmed"),
    ),
    "cmdi_candidate_parameter": FindingDefinition(
        "cmdi_candidate_parameter",
        "Probable command-injection parameter",
        "MEDIUM",
        0.60,
        "command_injection",
        "A parameter scored as a likely OS command-injection point from its name, value shape and endpoint context.",
        ("https://owasp.org/www-community/attacks/Command_Injection",
         "https://portswigger.net/web-security/os-command-injection"),
        "candidate",
    ...
    "framework_debug_enabled": FindingDefinition(
        ...
        0.85,
        "framework_debug_enabled",
        "Debug mode is on (Django DEBUG=True, Flask/Werkzeug debugger, Laravel Ignition…), exposing secrets/source and sometimes allowing code execution.",
        REFERENCE_SETS["error_disclosure"],
        "candidate",
        ("error-disclosure", "debug", "misconfiguration"),
    ),
    "sql_error_disclosure": FindingDefinition(
        "sql_error_disclosure",
        "SQL error message disclosed",
        "HIGH",
        0.80,
        "sql_injection",
        "A database error message was returned, revealing the DBMS and indicating error-based SQL injection may be possible.",
        REFERENCE_SETS["error_disclosure"],
        "candidate",
        ("sql-injection", "error-disclosure"),
    ),
    "server_error_disclosure": FindingDefinition(
        "server_error_disclosure",
        "Verbose server error / stack trace disclosed",
        "MEDIUM",
        0.75,
        "server_error_disclosure",
        "A stack trace or verbose error leaked framework, file paths and internal implementation details.",
        REFERENCE_SETS["error_disclosure"],
        "passive",
        ("error-disclosure", "information-leak"),
    ),
    "server_5xx_hotspot": FindingDefinition(
        "server_5xx_hotspot",
        "Frequent 5xx server errors",
        "MEDIUM",
        0.70,
        "server_5xx_hotspot",
        "A host returned many 5xx responses during the scan, indicating server-side faults.",
        REFERENCE_SETS["error_disclosure"],
        "passive",
        ("error", "stability"),
    ),
    "server_401_hotspot": FindingDefinition(
        "server_401_hotspot",
        "Large 401-protected surface",
        "INFO",
        0.70,
        "server_401_hotspot",
        "A host returned many 401 Unauthorized responses, marking a large authentication-protected surface.",
        REFERENCE_SETS["error_disclosure"],
        "passive",
        ("auth", "surface"),
    ),
