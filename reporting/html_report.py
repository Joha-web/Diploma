...
class HTMLReportGenerator:
    ...
    def _build_context(self, r: dict, ai_analysis: str) -> dict:
        ...
        active_probe_sections = [
            ...
            ("ssrf-probe", "SSRF Findings", r.get("ssrf_probe", {})),
            ("file-inclusion", "LFI / RFI Findings", r.get("file_inclusion", {})),
            ("command-injection", "Command Injection Findings", r.get("command_injection", {})),
        ...
        extra_finding_sections = [
            ...
            ("server-errors", "Server-Side Error Findings", r.get("error_analyzer", {}).get("findings", [])),
    ...
    def _build_finding_descriptions(r: dict) -> dict:
        ...
        return {
            ...
            "server_errors": {"title": "Server-Side Errors", "impact": "Database errors and stack traces leak the DBMS, frameworks, file paths and queries (aiding SQL injection and recon); frequent 5xx responses indicate fragile/faulting endpoints, and large 401 surfaces map the authenticated attack surface.", "detection": "5xx and 401 responses are aggregated per host from the session audit log; SQL-error and stack-trace signatures are matched against baseline responses and a small error-provoking probe (a lone quote) on parameterised endpoints.", "remediation": "Return generic error pages and disable debug/verbose errors in production. Use parameterized queries. Investigate the endpoints driving repeated 5xx faults."},
            "command_injection": {"title": "OS Command Injection", "impact": "User input reaches an OS shell, letting an attacker run arbitrary commands on the server — full host compromise, data theft, pivoting.", "detection": "Parameters are scored for command-injection likelihood (name like cmd/exec/ping/host, host/IP values, /ping or /exec endpoints). Likely params get an in-band echo+arithmetic probe, then commix (-p) and ffuf with the SecLists command-injection-commix wordlist (matching the executed MARKER<sum>MARKER echo pattern). A hit is CRITICAL.", "remediation": "Never pass user input to a shell. Use language-native APIs with argument arrays (no shell). Strictly allowlist values; reject shell metacharacters."},
            "file_inclusion": {"title": "Local / Remote File Inclusion (LFI/RFI)", "impact": "LFI lets an attacker read arbitrary server files (/etc/passwd, configs, source, secrets) and can escalate to code execution via log poisoning or PHP wrappers; RFI lets them include and execute remote code.", "detection": "File-inclusion-prone parameters are fuzzed with ffuf using the SecLists LFI wordlists (LFI-Jhaddix, LFI-linux-and-windows); ffuf's -mr keeps only responses containing real file signatures (root:x:0:0:, win.ini, PHP source). RFI/URL-wrapper inclusion is probed with a data:// marker — reflection proves allow_url_include is on.", "remediation": "Never pass user input to file/include APIs. Use allowlists of permitted files. Disable allow_url_include/allow_url_fopen. Canonicalize and confine paths to a safe base directory."},
            "ssrf_probe": {"title": "Server-Side Request Forgery (SSRF)", "impact": "Attackers can make the server issue requests to internal services, cloud metadata endpoints (169.254.169.254) or arbitrary hosts, leaking credentials and reaching otherwise-unreachable systems.", "detection": "Every discovered parameter is scored for SSRF likelihood from its name (url/redirect/webhook/proxy/image…), its value shape (already a URL/host?), and the endpoint path (/proxy, /fetch, /preview…); injection_probe SSRF hits are force-promoted. High-scoring candidates are confirmed automatically with SSRFmap when it is installed — an SSRFmap hit on internal services/metadata is HIGH; unconfirmed candidates carry their score and reasons.", "remediation": "Allowlist outbound destinations and schemes. Resolve and validate user-supplied URLs server-side. Block link-local/metadata ranges. Disable unused URL schemes (file, gopher, dict)."},
